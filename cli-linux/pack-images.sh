...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -t | --image-tag )
        ...
        shift 2 ;;
    -b | --build-solution )
        build_solution='yes'; shift 2;;
    -p | --docker-project )
       docker_project="$2"; shift 2;;
    --skip-image-push )
        push_images=''; shift 2;;
    --clear-images )
       clear_images='yes'; shift;;
    -h | --help )
        usage; exit 1 ;;
...
if [[ $build_solution ]]; then
    echo "#################### Building surging sample solution ####################"
    dotnet publish -o obj/Docker/publish ${workdir}/../sln/SunWin.Ifoc.Service.sln
fi

echo "指定docker镜像的tag为${image_tag}"
export TAG=$image_tag
echo "#################### Building surging sample Docker images ####################"
docker-compose -p .. -f ${workdir}/../docker-compose/hl/docker-compose.yml build
